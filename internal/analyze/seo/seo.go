...
func (a Analyzer) analyzePage(p *crawler.Page) []analyze.Issue {
	...
	switch {
	case title == "":
		add(analyze.Error, "missing-title", "Page has no <title>", nil)
	case len(title) < titleMin:
		add(analyze.Warning, "short-title", "Title is very short", map[string]any{"length": len(title), "title": title})
	case len(title) > titleMax:
		add(analyze.Warning, "long-title", "Title may be truncated in SERPs", map[string]any{"length": len(title), "title": title})
	}
	...
	case !hasDesc || strings.TrimSpace(desc) == "":
		add(analyze.Warning, "missing-meta-description", "Page has no meta description", nil)
	case len(desc) < descMin:
		add(analyze.Info, "short-meta-description", "Meta description is short", map[string]any{"length": len(desc)})
	case len(desc) > descMax:
		add(analyze.Info, "long-meta-description", "Meta description may be truncated", map[string]any{"length": len(desc)})
	}
	...
	if robots, ok := metaContent(doc, "name", "robots"); ok {
		...
		if strings.Contains(low, "noindex") {
			add(analyze.Warning, "meta-noindex", "Page is marked noindex", map[string]any{"robots": robots})
		}
		if strings.Contains(low, "nofollow") {
			add(analyze.Info, "meta-nofollow", "Page is marked nofollow", map[string]any{"robots": robots})
		}
	}
	...
	if href, ok := doc.Find(`head link[rel="canonical"]`).First().Attr("href"); ok && strings.TrimSpace(href) != "" {
		if doc.Find(`head link[rel="canonical"]`).Length() > 1 {
			add(analyze.Warning, "multiple-canonical", "Multiple canonical links found", nil)
		}
	} else {
		add(analyze.Info, "missing-canonical", "Page has no canonical link", nil)
	}
	...
	switch h1.Length() {
	case 0:
		add(analyze.Warning, "missing-h1", "Page has no <h1>", nil)
	case 1:
	default:
		add(analyze.Info, "multiple-h1", "Page has multiple <h1> elements", map[string]any{"count": h1.Length()})
	}

	// html lang.
	if lang, ok := doc.Find("html").First().Attr("lang"); !ok || strings.TrimSpace(lang) == "" {
		add(analyze.Info, "missing-lang", "<html> has no lang attribute", nil)
	}

	// Viewport.
	if _, ok := metaContent(doc, "name", "viewport"); !ok {
		add(analyze.Info, "missing-viewport", "Page has no viewport meta (mobile-friendliness)", nil)
	}

	// Charset.
	if doc.Find(`head meta[charset]`).Length() == 0 {
		if _, ok := metaHTTPEquiv(doc, "content-type"); !ok {
			add(analyze.Info, "missing-charset", "Page declares no character set", nil)
		}
	}

	// Open Graph / Twitter cards.
	if doc.Find(`head meta[property^="og:"]`).Length() == 0 {
		add(analyze.Info, "missing-opengraph", "No OpenGraph tags (affects social sharing)", nil)
	}
